...
def _init_alarm_settings() -> list[cp.AlarmSettings]:
    return [cp.AlarmSettings()]
...
@dataclass
class FlysightConfig:
    """
    Helper representation for the FlySight configuration parameters.
    ...
    alarm_settings: list[cp.AlarmSettings] = field(default_factory=_init_alarm_settings)
    alt_settings: cp.AltitudeSettings = field(default_factory=cp.AltitudeSettings)
    silence_window_settings: list[cp.SilenceWindowSettings] = field(
